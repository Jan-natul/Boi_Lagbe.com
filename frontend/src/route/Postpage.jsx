...
const PostPage = () => {
    ...
    const fetchPostAndStatus = async () => {
      try {
        ...
        if (user) {
            ...
            const savedRes = await fetch(`http://localhost:8080/user/is-saved/${id}`, {
  ...
  const handleSaveToggle = async () => {
      ...
      try {
          const token = localStorage.getItem('token');
          const response = await fetch(`http://localhost:8080/user/save/${id}`, {
              method: 'PUT',
              headers: { 
          ...
          <div className="w-full md:w-1/3">
            <img
              src={
                 ...
                 ? post.image 
                 : `http://localhost:8080/images/${post.image}`
              }
              alt={post.title}
